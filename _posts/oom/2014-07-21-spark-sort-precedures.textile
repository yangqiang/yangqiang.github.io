...
worker 拆开 task 执行的时候会调用 RDD.iterator 来获取 RDD 的数据。（如果没有获得，就会去计算以生成数据：getOrCompute）
...
基于 scala 中 akka.actor 技术，天然支持分布式编程。

# 集群启动的时候各 worker 节点会初始化 SparkContext 对象，好比仓库主管给每个手下一把分库的钥匙。
# 收到 task 之后， SparkContext 对象根据任务去创建 RDD 并进行计算。这一块可以理解为：主管将任务切分为 1 2 3 三块，分别分配给三个手下（ worker ）去执行，手下执行任务的时候会根据任务编号去仓库取出货物进行处理。一个好的主管，会根据手下离仓库的距离来分配任务，使每个手下能就近取货进行工作。
# 主管以短信的方式发送任务，比如“拆卸”、“烘干”、“包装”，三个手下按步骤一一执行任务。
# 有些任务需要协同完成，比如主管想知道一批货里面最重的货物有多重，这就要求各手下发短信协调完成。对于 shuffle 类计算，则需要为分仓库之间建立传送带，保证每两个仓库之间都能相互传送货物。
